...
```json
{
  "mcpServers": {
    "remote tiny-mcp-server (HTTP Streamable)": {
      ...
      "args": [
        "mcp-remote",
        "https://tiny-mcp-server--<your_domain>.replit.dev/mcp?tiny_mcp_server_api_key=<secret_key_here>"
      ]
    }
  }
}
```**Note:** 
Claude Desktop requires a non-self-signed HTTPS certificate for remote servers. I choose to deploy the remote HTTP Streamable MCP Server using **Replit**, which provides public HTTPS URL with valid certificate (more on this in \'Deploying to Replit\')
...
### Securing Your Remote Server with API Key Authentication

Since the public URL is accessible to anyone on the internet, you should **protect it with an API key**:
  - How it works:
    - Requests without `?tiny_mcp_server_api_key=...` → `401 Unauthorized`
    - Requests with wrong key → `401 Unauthorized`
    - Requests with correct key → MCP server processes normally
    - **Optional**: If `TINY_MCP_SERVER_API_KEY` is not set, authentication is **skipped** (useful for local testing)
  - ✅ The API key is only checked on the `/mcp` endpoint (health check on `/` is always public)
  - ✅ Store your secret key only in Replit Secrets, not in version control
  - ✅ Use a strong, random key (e.g., generated by `openssl rand -base64 32`)
  - ⚠️ The key travels in the URL, so always use HTTPS (Replit provides this)

**Set the API key in Replit:**:
   - Go to your Replit project → Secrets (lock icon)
   - Add a new secret: `TINY_MCP_SERVER_API_KEY=your_secret_key_here`
   - The server will read this and enforce authentication

**Testing Replit's secured deploiement:**
When testing the Replit server with curl, you should also provide the API key : 
   ```bash
   curl -X POST "https://tiny-mcp-server--<your_domain>.replit.dev/mcp?tiny_mcp_server_api_key=<secret_key_here>" \
     -H "Content-Type: application/json" \
     -H "Accept: application/json, text/event-stream" \
     -d '{"jsonrpc":"2.0",...}'
   ```

**Testing a local secured deploiement:**
When testing the server on your local machine, you should either:
  - for simplicity : start the server without any API key, so that there will be no securoty check:
  ```bash
  node dist/index.js --remote
  ```
  - for more real life usecase, start the server with an API key:
  ```bash
  TINY_MCP_SERVER_API_KEY=test123 node dist/index.js --remote
  ```
