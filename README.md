...
```json
{
  "mcpServers": {
    "remote tiny-mcp-server (HTTP Streamable)": {
      ...
      "args": [
        "mcp-remote",
        "https://8cf40eb2-f246-4e9f-b3f3-96bcb45db89d-00-z7uripd5th2g.kirk.replit.dev/mcp?tiny_mcp_server_api_key=your_secret_key_here"
      ]
    }
  }
}
...
- as Replit provides a public url, the remote MCP server is secured through a `tiny_mcp_server_api_key` secret:
  - How it works:
...
When testing the Replit server with curl, you should also provide the API key : 
   ```bash
   curl -X POST "https://your-domain.replit.dev/mcp?tiny_mcp_server_api_key=your_secret_key_here" \
